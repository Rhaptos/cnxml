...
<!-- ********************************************************************
  ...
  <xsl:template match="cnx:table">
	...
	<xsl:when test="cnx:categories">
	  <table border="1" class="old-table">
	    ...
	    <!--Outputs the content of ELEMs in the order they are listed
	    within each GROUP.-->
  ...
  <xsl:template match="cnx:thead|cnx:tfoot">
    <xsl:element name="{name(.)}">
      <xsl:if test="parent::cnx:tgroup/@align">
	<xsl:attribute name="align">
	  <xsl:value-of select="parent::cnx:tgroup/@align"/>
	</xsl:attribute>
      </xsl:if>
      <xsl:if test="@char">
  ...
  <xsl:template match="cnx:thead/cnx:row/cnx:entry">
  ...
  <xsl:template match="cnx:tfoot/cnx:row/cnx:entry">
    <xsl:call-template name="process.cell">
      <xsl:with-param name="cellgi">th</xsl:with-param>
    </xsl:call-template>
  </xsl:template>
  ...
  <xsl:template name="entry.colnum">
	...
	<xsl:variable name="colspec"
		      select="$entry/ancestor::cnx:tgroup/cnx:colspec[@colname=$colname]"/>
	...
	<xsl:variable name="colspec"
		      select="$entry/ancestor::cnx:tgroup/cnx:colspec[@colname=$namest]"/>
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$colspec"/>
	</xsl:call-template>
      </xsl:when>
  ...
  <xsl:template name="entry.ending.colnum">
	...
	<xsl:variable name="colspec"
		      select="$entry/ancestor::cnx:tgroup/cnx:colspec[@colname=$nameend]"/>
	<xsl:call-template name="colspec.colnum">
	  <xsl:with-param name="colspec" select="$colspec"/>
	</xsl:call-template>
      </xsl:when>
  ...
  <xsl:template name="colspec.colwidth">
    ...
    <xsl:param name="colspecs" select="../../../../cnx:tgroup/cnx:colspec"/>
    <xsl:param name="count">1</xsl:param>
    <xsl:choose>
