...
<xsl:stylesheet version="1.0" 
  ...
  <!-- Default to *not* displaying the 'nomathml' blurb -->
  <xsl:param name="nomath" select="0" />
  ...
  <xsl:variable name="created"><xsl:value-of select="//cnx:created" /></xsl:variable>
  <xsl:variable name="revised"><xsl:value-of select="//cnx:revised" /></xsl:variable>
  <xsl:variable name="version"><xsl:value-of select="//cnx:version" /></xsl:variable>
  ...
  <xsl:template match="cnx:module">
    <body id="{@id}"
      ...
      <xsl:if test="$nomath and //m:math|//cnx:equation">
	<div class="nomathml">
	  <a name="nomathml">
	...
	<xsl:apply-templates select="//cnx:authorlist|//cnx:maintainerlist" />
      </div>
  ...
  <xsl:template match="cnx:metadata">
    <div class="metadata">
      <xsl:apply-templates select="*[not(self::cnx:authorlist|self::cnx:maintainerlist)]" />
    </div>
  </xsl:template>
