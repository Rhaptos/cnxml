...
<!--
        <script language="javascript" src="http://rhaptos.ece.rice.edu/cnxml/0.4/scripts/exercise.js" />
...
-->
	...
	<link rel="stylesheet" title="Default" type="text/css" href="/cnxml/0.4/style/netscape4/netscape4.css" />
		...
		<tr>
		  ...
		  <td class="metadata-and-content">
			...
			<div class="latest">Note: You are viewing an old version of this document.
		      ...
		      <xsl:apply-templates select="//md:abstract|//md:objectives"/>
			...
			<div class="footnote">
	    		    ...
	    		    <span class="footnote-before"><xsl:value-of select="$footnote-number" />: </span>
	  		  </a>
			  <xsl:apply-templates />
			</div>
		      </xsl:for-each>
			...
			<table border="0" cellspacing="0" cellpadding="0" class="authorlist-maintainerlist">
			  <tr>
			    <td class="authorlist-before" valign="top">
			      <span class="authorlist-before">Written By: </span>
			    </td>
			    <td class="authorlist">
			      <xsl:apply-templates select="cnx:metadata/md:authorlist" />
			    </td>
			  </tr>
			  <tr>
			    <td class="maintainerlist-before" valign="top">
			      <span class="maintainerlist-before">Maintained By: </span>
			    </td>
			    <td class="maintainerlist">
			      <xsl:apply-templates select="cnx:metadata/md:maintainerlist" />
			    </td>
			  </tr>
			</table>
		      </div>
  ...
  <xsl:template match="cnx:example">
	...
	<tr>
	  <xsl:choose>
	    <xsl:when test="parent::cnx:definition|parent::cnx:rule|parent::cnx:problem|parent::cnx:solution">
	      <td width="72"><img src="/images/spacer.gif" width="72" height="1" /></td>
	    </xsl:when>
	    <xsl:otherwise>
	  ...
	  <td class="example" width="100%">
	    <table class="inner-example-table" border="1" cellpadding="7" cellspacing="0" width="100%">
	      <tr>
		<td>
		  <xsl:choose>
		    <xsl:when test="child::*[position()=1 and local-name()='name']">
		      <div class="example-before">
			<span class="example-before">EXAMPLE: </span>
  ...
  <xsl:template match="cnx:definition">
    <div class='definition'>
      <div class="term-before">
	<span class="term-before">TERM: </span>
  ...
  <xsl:template match="cnx:rule">
    <div class='rule' type='{@type}'>
      <span class="rule-before"><xsl:value-of select="translate(@type, $lower, $upper)"/>: </span>
  ...
  <xsl:template match="cnx:problem">
    <div class="problem">
      <span class="problem-before">EXERCISE
  ...
  <xsl:template match="cnx:solution">
    ...
    <div class="solution">   <!--onclick="hideSolution('{@id}')"-->
      <span class="solution-before">SOLUTION: </span>
      <xsl:apply-templates />
    </div>
  </xsl:template>
	  ...
	  <td width="100%">
		...
		<td class="note">
		  <div class="note">
			...
			<span class="note-without-type-before">NOTE: </span> 
			<xsl:apply-templates/> 
		      </xsl:when>
		      <xsl:otherwise>
			<xsl:attribute name="title">
			...
			<span class="note-before"><xsl:value-of select="translate(@type, $lower, $upper)"/>: </span>
			<xsl:apply-templates/>
		      </xsl:otherwise>
		    </xsl:choose>
		  </div>
	...
	<table class="equation" width="100%" cellspacing="0" cellpadding="0" border="0">
	  <xsl:choose>
	    <xsl:when test="cnx:name">
	      <tr>  
		<td class="name" colspan="2"> <xsl:value-of select="cnx:name/text()" /> </td> 
	      </tr>
	      <tr>
		<td class="equation" align="center">
		  <div class="equation">
		    [<a class="equation" href="#no-mathml">Math display error.  See note.</a>]
		  </div>
		</td>  
  ...
  <!-- This template added to resolve problem w/ corecnxml.xsl, which 
       was putting a <br> tag after subfigures' caption -->
  ...
  <xsl:template match="cnx:figure">	
	...
	<xsl:choose>
	  <xsl:when test="cnx:subfigure">
		...
		<tr>
		  <td class="figure-name" colspan="{$subfigure-quantity}">
		    <span class="figure-name">
		      <span class="figure-name-before">
		...
		<tr>
		  <xsl:for-each select="cnx:subfigure">
		    <td class="subfigure-name" valign="bottom">
		      <span class="subfigure-name">
			<span class="subfigure-name-before">
		...
		<tr>
		  <xsl:for-each select="cnx:subfigure">
			...
			<span class="subfigure-caption">
			  <span class="subfigure-caption-before">Subfigure <xsl:number level="any" count="cnx:figure" />.<xsl:number count="cnx:subfigure"/>: </span>
		...
		<xsl:if test="cnx:caption">
		  ...
		  <tr>
		    <td class="caption" colspan="{$subfigure-quantity}">
		      <span class="caption">
			<span class="caption-before">Figure <xsl:number level="any" count="cnx:figure" />: </span> 
			<xsl:apply-templates select="cnx:caption" />
		      </span>
		    </td>
		  </tr>
