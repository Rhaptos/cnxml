...
<xsl:stylesheet version="1.0" 
  ...
  FIX ME: I don't think this is right.  This will work for sections,
	but what about subsections.
Shouldn't it be:
<xsl:value-of select="$grpnumb"/>.<xsl:number level="multiple"
      from="rdf:li/rdf:Description[child::cnxml:class[normalize-space(.)='pageview']]"
      count="cnx:section"/>
...
<!--stopped here-->
  ...
  <xsl:template match="cnx:figure|cnx:exercise|cnx:definition|cnx:equation">
    <xsl:param name='grpnumb'/>
    <xsl:param name='grptop'/>
