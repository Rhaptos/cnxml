...
<!-- This file will only work with modules being printed alone in the 0.3 -->
<!-- namespace.  You must change code to include this properly with a mixed -->
<!-- cnxml namespace environment. -->
...
<!-- SECTION/NAME -->
  <xsl:template match="cnx:module/cnx:section/cnx:name">
...
<!-- SECTION/SECTION/NAME -->
  <xsl:template match="cnx:module/cnx:section/cnx:section/cnx:name">
    <fo:block keep-with-next="always"
	      font-size="14pt"
	      font-weight="bold"
              space-before="5pt"
...
<!-- PARA  -->
  ...
  <xsl:template match="cnx:section/cnx:para|cnx:module/cnx:para">
...
<!-- NOTE :) -->
  ...
  <xsl:template match="cnx:note[@type='footnote']">
	...
	<fo:inline font-size="10pt" 
	  ...
	  <xsl:number format="1" 
		      level="any" 
		      count="cnx:solution|cnx:note[@type='footnote']"/>
	</fo:inline>
      </fo:footnote-citation>
      <fo:footnote-body>
...
<!-- FIGURE ?:) -->
  <xsl:template match="cnx:figure">
    <fo:block keep-together.within-page="always"
	      ...
	      space-after=".3cm"
	      text-align="center">
      <xsl:call-template name="figure-guts-0.3"/>
    </fo:block>
  </xsl:template>


  <xsl:template match="cnx:section/cnx:figure|cnx:module/cnx:figure">
 ...
 <fo:block text-align="center">
  ...
  <xsl:template name="figure-guts-0.3">
...
<!-- MEDIA :) -->
  <xsl:template match="cnx:media[starts-with(@type,'image')]">
	...
	select="translate(ancestor::cnx:module/@id,'m','')"/>/<xsl:value-of
	select="@src"/>
	</xsl:attribute>
      </fo:external-graphic>
    </fo:block>
...
<!--  <xsl:template match="cnx:cnxn[text()]">
  ...
  <xsl:template name="refnumb">
	...
	<xsl:variable name="type" select="@type"/>
	<xsl:for-each select="key('id',@target)"><xsl:value-of
	select="@type"/>
	  <xsl:text> </xsl:text>
	  <xsl:choose>
	    <xsl:when test="@number">
  ...
  <xsl:template match="cnx:cnxn[@target and @module]">
	...
	in the book. --><xsl:call-template name="refnumb"/></xsl:when>
...
<!-- TABLE -->
  <xsl:template match="cnx:table">
    <fo:block text-align="center">
   I'm sorry, this table is written in cnxml 0.3.  Please convert to 0.3.5.
      </fo:block>
  </xsl:template>
