...
<!-- ==================================================================== -->
  ...
  <xsl:template match="cnx:table/cnx:name">
  ...
  <fo:table-body>
    <xsl:choose>
      <xsl:when test="$frame='top'">
        <xsl:choose>
          <xsl:when test="preceding-sibling::cnx:thead">
          </xsl:when>
          <xsl:otherwise>
            <xsl:call-template name="border">
              <xsl:with-param name="side" select="'top'"/>
      ...
      <xsl:when test="$frame='bottom'">
        <xsl:choose>
          <xsl:when test="preceding-sibling::cnx:tfoot">
          </xsl:when>
          <xsl:otherwise>
            <xsl:call-template name="border">
...
<xsl:template name="process.cell">
  <xsl:variable name="row" select="parent::cnx:row"/>
  <xsl:variable name="group" select="$row/parent::*[1]"/>
  <xsl:variable name="frame" select="ancestor::cnx:tgroup/parent::*/@frame"/>
...
-->
  ...
  <fo:table-cell>
    ...
    <xsl:if test="@namest">
      ...
      <fo:block margin=".1cm">
	<xsl:if test="ancestor::cnx:thead">
...
<xsl:template name="generate.col.raw">
  ...
  <xsl:param name="colspecs" select="./cnx:colspec"/>
