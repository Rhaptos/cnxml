...
<!-- ==================================================================== -->
  ...
  <xsl:template match="*[local-name()='colspec']"></xsl:template>
  
  <xsl:template match="*[local-name()='spanspec']"></xsl:template>
  ...
  <fo:table-body>
    <xsl:choose>
      <xsl:when test="$frame='top'">
        <xsl:choose>
          <xsl:when test="preceding-sibling::*[local-name()='thead']">
          </xsl:when>
          <xsl:otherwise>
            <xsl:call-template name="border">
              <xsl:with-param name="side" select="'top'"/>
      ...
      <xsl:when test="$frame='bottom'">
        <xsl:choose>
          <xsl:when test="preceding-sibling::*[local-name()='tfoot']">
          </xsl:when>
          <xsl:otherwise>
            <xsl:call-template name="border">
...
<xsl:template match="*[local-name()='thead']/*[local-name()='row']/*[local-name()='entry']">
  <xsl:call-template name="process.cell"/>
</xsl:template>

<xsl:template match="*[local-name()='tbody']/*[local-name()='row']/*[local-name()='entry']">
  <xsl:call-template name="process.cell"/>
</xsl:template>

<xsl:template match="*[local-name()='tfoot']/*[local-name()='row']/*[local-name()='entry']">
  <xsl:call-template name="process.cell"/>
</xsl:template>

<xsl:template name="process.cell">
  <xsl:variable name="row" select="parent::*[local-name()='row']"/>
  <xsl:variable name="group" select="$row/parent::*[1]"/>
  <xsl:variable name="frame" select="ancestor::*[local-name()='tgroup']/parent::*/@frame"/>
...
<xsl:template name="calc.column.width">
  ...
  <!--<xsl:if test="contains($colwidth, '*')">
	...
	<xsl:value-of select="100 div @cols"/>%</xsl:when>
      <xsl:otherwise>
