<?xml version="1.0" encoding="utf-8"?>
  ...
  <xsl:template match="/">
	...
	<fo:page-sequence-master master-name="contents">
      ...
      <!--The COVER-->
      <fo:page-sequence master-name="cover" force-page-count="end-on-even">
	<fo:flow flow-name="xsl-region-body">
	 ...
	 <!--   </fo:list-block> -->
      ...
      <!--INNER COVER PAGE-->
      <fo:page-sequence master-name="cover" initial-page-number="1" >
	<fo:flow flow-name="xsl-region-body">
	...
	<fo:flow>
	  ...
	  <fo:block  space-before="1in">
	    This course was created using The Connexions Project (http://cnx.rice.edu).
	  </fo:block>
	</fo:flow>
      </fo:page-sequence>
	...
	<fo:flow>
	  <fo:block text-align="begin"
	    font-size="24pt"
	    font-weight="bold"
	    space-before=".5in">Contributing Authors</fo:block>
	  
	  <fo:block text-size="14pt" text-align="begin"
	    space-after=".5in">The following authors wrote material
	    that appears in this book.</fo:block>
...
<!--i am the modnum
		      <xsl:value-of select="key('modnumb',@id)"/>that was me!
		      <xsl:choose>-->
...
<!--Otherwise don't.-->
  ...
  <xsl:template
    ...
    <fo:block id="{@about}">
      ...
      <xsl:for-each select="cnx:pre/rdf:Seq/rdf:li/rdf:Description/cnx:content/cnxml:module">
	<xsl:apply-templates/>
      </xsl:for-each>

      <!--Module itself (without a name)-->
      <xsl:apply-templates select="rdf:Description/cnx:content/cnxml:module"/>

      <!--Following Segues and Problemsets (without a name)-->
      <xsl:for-each select="cnx:post/rdf:Seq/rdf:li/rdf:Description/cnx:content/cnxml:module">
	<xsl:apply-templates/>
      </xsl:for-each>
