	...
	<fo:flow flow-name="xsl-region-body">
	  ...
	  <fo:block font-size="12pt"
	   ...
	   <!--Second Step in creating the authorlist.  Sort by surname.-->
		...
		<xsl:variable name="author" select="substring-after('*',@id)"/>
		<xsl:if test="not(preceding::cnxml:author[substring-after('*',@id)=$author])">
		  <fo:block>
		    <!-- normalize here?  or at a later time, ie. in tofo -->
		    <xsl:copy-of select="cnxml:firstname"/> 
		    <xsl:copy-of select="cnxml:surname"/>
		  </fo:block>
		</xsl:if>
	      </xsl:for-each>
	    </fo:block>
