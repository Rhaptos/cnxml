...
<xsl:template match="m:apply[child::*[position()=1 and local-name()='apply']]">  <!-- when the function itself is defined by other functions: (F+G)(x) -->
  ...
  <xsl:when test="count(child::*)>=2">
    <mrow>
      <mfenced separators=""><xsl:apply-templates
      select="child::*[position()=1]"/></mfenced>
      <mfenced>
<xsl:apply-templates select="child::*[position()!=1]"/></mfenced>
...
<xsl:template match="m:set">
  <mrow>
    ...
    <xsl:when test="m:condition"> <!-- set defined by a condition-->
      <mo>{</mo><mrow>
<mfenced open="" close=""><xsl:apply-templates select="m:bvar"/></mfenced><mo>|</mo><xsl:apply-templates select="m:condition"/></mrow><mo>}</mo>
    </xsl:when>
