...
<!-- METADATA :) -->
  <!-- FIX ME: i think this could be simplified to "mdml:*" -->
  <xsl:template match="mdml:authorlist|mdml:maintainerlist|mdml:keywordlist|mdml:objectives|mdml:abstract|mdml:created|mdml:revised|mdml:version" />
...
<!-- MODULE/NAME :) -->
  <xsl:template match="module/name | cnx:module/cnx:name">
...
<!-- PARA  -->
  ...
  <xsl:template match="cnx:para[following-sibling::cnx:equation]">
    <fo:block text-align="justify"
	      text-indent="15pt">
      <xsl:copy-of select="@id"/>
      <xsl:apply-templates/>
...
<!-- FIGURE ?:) -->
  <xsl:template match="cnx:figure">
    <fo:block keep-together.within-page="always"
	      ...
	      text-align="center">
      <xsl:copy-of select="@id"/>
      <xsl:call-template name="figure-guts"/>
    </fo:block>
  </xsl:template>
  ...
  <xsl:template name="figure-guts">
    ...
    <xsl:choose>
      <!--=-= YES CAPTION: -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
      <xsl:when test="descendant::cnx:caption">
	<fo:block text-align="justify"
		  margin-left="2.5cm"
		  margin-right="2.5cm"
		  font-size="10pt">
...
<!-- MEDIA :) -->
  <xsl:template match="cnx:media[starts-with(@type,'image')]">
    <fo:block text-align="center">
      <xsl:copy-of select="@id"/>
      <fo:external-graphic>
	<xsl:attribute name="src">http://cnx.rice.edu/modules/<xsl:value-of select="ancestor::*[local-name()='module']/@id"/>/latest/<xsl:value-of select="@src"/>
	</xsl:attribute>
      </fo:external-graphic>
    </fo:block>
  </xsl:template>
  ...
  <xsl:template match="cnx:media[@type='application/x-java-applet']">
    <fo:block space-before="1cm"
      ...
      http://cnx.rice.edu/modules/<xsl:value-of select="ancestor::*[local-name()='module']/@id"/>/latest/
    </fo:block>
  </xsl:template>
  ...
  <xsl:template match="cnx:equation">
	...
	<m:mtable class="equation"> 
	  <m:mlabeledtr>
	    ...
	    <m:mtd>
	      <xsl:apply-templates  select="child::*[not(self::cnx:name)]" />
	    </m:mtd>
	  </m:mlabeledtr>
	</m:mtable>
      </m:math>
...
<!--=-=-=- both target and module =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-->
  <xsl:template match="cnx:cnxn[@target and @module]">
    ...
    <xsl:choose>
      ...
      <xsl:when test="key('id',@module)"><xsl:call-template name="refnumb"/></xsl:when>
  ...
  <xsl:template match="cnx:cnxn[@module and not(@target)]">
	...
	<fo:inline font-size="10pt">
	  <xsl:if test="text()|node()">
	    <xsl:text> (</xsl:text> 
	  </xsl:if>http://cnx.rice.edu/modules/<xsl:value-of select="@module"/>/latest/<xsl:if test="text()|node()">
	    <xsl:text>)</xsl:text> 
	  </xsl:if>
	</fo:inline>
      </xsl:otherwise>
...
<!--  LINK :)  -->
  <xsl:template match="cnx:link">
    <xsl:apply-templates/>
    <fo:inline font-size="10pt"> (<xsl:value-of select="@src"/>)</fo:inline>
  </xsl:template>
