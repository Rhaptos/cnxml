<?xml version="1.0" encoding="utf-8"?>
  ...
  <xsl:template match="ind:indexlist">
	...
	<xsl:when test="translate(substring(normalize-space(.),1,1),
	  ...
	  <xsl:if test="not(position()=1)"><!--if not the first word(which
	  hasn't been started yet) then: -->
	    ...
	    before calling print-letter again to print out the next letter set
	    --> 
	  </xsl:if>
    ...
    <xsl:text disable-output-escaping="yes">
    &lt;/fo:block></xsl:text> <!-- ends the very last word's fo:block -->
  </xsl:template>
